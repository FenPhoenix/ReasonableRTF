...
internal static class CodeWriters
{
    internal sealed class IndentingWriter
    {
        ...
        internal void WL(string str = "")
        {
            ...
            else if (strT.StartsWithOPlusWhiteSpace("switch"))
            {
                _inSwitchStatement = true;
                _inFirstCaseStatement = true;
            }
            else if (strT.StartsWithOPlusWhiteSpace("case"))
            {
                _nextIndent++;
                if (_inSwitchStatement)
                {
            ...
            bool noIndent = strT.IsWhiteSpace() ||
                            (strT.StartsWithO("#") &&
                             !strT.StartsWithOPlusWhiteSpace("#region") &&
                             strT != "#region" &&
                             !strT.StartsWithOPlusWhiteSpace("#endregion") &&
                             strT != "#endregion");
